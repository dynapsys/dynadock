...
class TestLANNetworkManager:
    ...
    def lan_manager(self, project_dir):
        ...
        with patch.object(
            LANNetworkManager, "_auto_detect_interface", return_value="eth0"
        ):
    ...
    def test_auto_detect_interface_fallback(
        self, mock_exists, mock_subprocess, project_dir
    ):
    ...
    def test_add_virtual_ip_success(self, mock_run, lan_manager):
        ...
        with (
            patch.object(lan_manager, "_announce_arp"),
    ...
    @patch.object(
        LANNetworkManager,
        "get_network_details",
        return_value=("192.168.1.10", "192.168.1.0", "24", "192.168.1.255"),
    )
    @patch.object(
        LANNetworkManager,
        "find_free_ips",
    ...
    def test_setup_services_lan_success(
        self, mock_add_ip, mock_find_ips, mock_get_network, mock_check_root, lan_manager
    ):
    ...
    def test_cleanup_all(self, lan_manager):
        ...
        with (
            patch.object(
                lan_manager,
                "_load_ip_tracking",
                return_value={"virtual_ips": [{"ip": "192.168.1.100"}]},
            ),
...
class TestLANNetworkManagerIntegration:
    ...
    @pytest.mark.skipif(
        not Path("/proc/sys/net/ipv4/ip_forward").exists(),
...
class TestLANNetworkManagerErrorHandling:
    ...
    def test_interface_not_found_error(self, tmp_path):
        ...
        with (
            patch.object(LANNetworkManager, "_auto_detect_interface") as mock_detect,
            patch.object(LANNetworkManager, "_interface_exists", return_value=False),
        ):

            mock_detect.side_effect = DynaDockNetworkError(
