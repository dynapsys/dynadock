...
async def test_domain_headless(url: str, timeout: int = 10, verbose: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            _setup_page_handlers(page, network_requests, errors, ssl_errors, console_logs)
            ...
            log(f"Strona załadowana w {load_time:.3f}s, status: {response.status if response else 'N/A'}")
                
            if response:
                log(f"Przetwarzanie pozytywnej odpowiedzi (status: {response.status})")
                result = await _process_successful_response(
                    page, response, load_time, safe_filename, 
                    network_requests, console_logs, errors, ssl_errors
                )
            else:
...
async def _process_successful_response(page, response, load_time, safe_filename, 
                                     network_requests, console_logs, errors, ssl_errors):
...
async def _process_failed_response(page, safe_filename, network_requests, errors, ssl_errors):
...
async def _process_exception(page, exception, safe_filename, network_requests, errors, ssl_errors):
