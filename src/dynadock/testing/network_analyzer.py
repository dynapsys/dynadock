...
def analyze_network_connectivity(url: str, verbose: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        dns_time = time.time() - start_time
        analysis['dns_resolution'] = {'success': False, 'error': str(e), 'time': dns_time}
        analysis['timing']['dns'] = dns_time
    ...
    log(f"Port scan complete: {len(open_ports)}/{len(common_ports)} open ({scan_time:.3f}s)")
        ...
        try:
            ...
            with socket.create_connection((hostname, port), timeout=5) as sock:
                with context.wrap_socket(sock, server_hostname=hostname) as ssock:
                    ...
                    subject_cn = analysis['ssl_cert_info']['subject'].get('commonName', 'Unknown')
                    issuer_org = analysis['ssl_cert_info']['issuer'].get('organizationName', 'Unknown')
                    log(f"SSL SUCCESS: CN='{subject_cn}', Issuer='{issuer_org}' ({ssl_time:.3f}s)")
        except Exception as e:
            ssl_time = time.time() - ssl_start
